...
Lambda.prototype.setup = function() {
  ...
  this._createSampleFile('event.js');
  console.log('Setup done. Edit the .env and event.js files as needed.');
};
...
Lambda.prototype.run = function(program) {
  ...
  var event = require(process.cwd() + '/event.js');
  var this_instance = this;
  
  event.feed( function(event) {
    this_instance._runHandler(handler, event);
  });
};
  ...
  var context = {
    ...
    fail: function(error) {
      ...
      process.exit(-1);
        ...
        console.log('done:\n' + util.inspect(Array.slice(arguments), {depth: 5}));
